...
class CircleCropper:
    ...
    def compute_median(self, pgrads, qgrads):
        """Compute and normalize the median curvature."""
        print("Computing median curvature...")

        # Ensure `qgrads` is the same size as `pgrads`
        if pgrads.shape != qgrads.shape:
            qgrads = cv2.resize(qgrads, (pgrads.shape[1], pgrads.shape[0]))
            
        #pgrads = cv2.GaussianBlur(pgrads, (25, 25), 1)
        #qgrads = cv2.GaussianBlur(qgrads, (25, 25), 1)

        # Compute Sobel derivatives
        ddepth = cv2.CV_32F
        Ixx = cv2.Sobel(pgrads, ddepth, 1, 0, ksize=1)
        Ixy = cv2.Sobel(pgrads, ddepth, 0, 1, ksize=1)
        Iyy = cv2.Sobel(qgrads, ddepth, 0, 1, ksize=1)
        Iyx = cv2.Sobel(qgrads, ddepth, 1, 0, ksize=1)

        # Compute mean curvature
        a = (1 + np.float_power(pgrads, 2)) * Iyy
        b = pgrads * qgrads * (Ixy + Iyx)
        c = (1 + np.float_power(qgrads, 2)) * Ixx
        d = np.float_power(1 + np.float_power(pgrads, 2) + np.float_power(qgrads, 2), 3 / 2)
        meangrad = (a - b + c) / (d + 1e-5)  # Add epsilon to avoid division by zero

        # Normalize result
        meangrad_norm = cv2.normalize(meangrad, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
        print("Median curvature computation complete.")
    ...
    def process_images(self):
        ...
        for image_file in image_files:
            ...
            for i, (x, y, r) in enumerate(self.circles, start=1):
                ...
                median_curvature = self.compute_median(gray, gray)
                ...
                output_cropped_filename = f"{base_name}_{folder_number}_roi{i}{ext}"
                output_median_filename = f"{base_name}_{folder_number}_roi{i}_median{ext}"
                output_blurred_filename = f"{base_name}_{folder_number}_roi{i}_blurred{ext}"

                cropped_path = os.path.join(self.output_folder, output_cropped_filename)
                median_path = os.path.join(self.output_folder, output_median_filename)
                blurred_path = os.path.join(self.output_folder, output_blurred_filename)

                # บันทึกภาพที่ crop และภาพมัธยฐาน
                cv2.imwrite(cropped_path, circle_only)
                cv2.imwrite(median_path, median_curvature)
                cv2.imwrite(blurred_path, img_blurred)

                print(f"Saved cropped image: {cropped_path}")
                print(f"Saved median curvature image: {median_path}")
                print(f"Saved blurred curvature image: {blurred_path}")
...
if __name__ == "__main__":
    ...
    for index in range(1, 5):
        folder_number = index
        input_folder = f"/home/mic/Camera_project/basler/Result_{folder_number}"
        output_folder = f"/home/mic/Camera_project/basler/ROI_{folder_number}"

        # ตำแหน่งวงกลมที่ต้องการ crop
        circles = [
            (375, 1430, 170),  # วงกลมที่ 1
            (870, 545, 170),   # วงกลมที่ 2
            (855, 1100, 170),   # วงกลมที่ 2
        ]
